...
export default function SupplierFilters({
    ...
    try {
      if (filteredSuppliers.length === 0) {
        toast({
          ...
          description: t("suppliers.noDataToExport"),
          variant: "destructive",
        });
        return;
      }
      ...
      const csvData = filteredSuppliers.map((supplier) => ({
        [t("suppliers.supplierName")]: supplier.name,
        [t("common.status")]: supplier.status ? t("common.active") : t("common.inactive"),
        [t("common.description")]: supplier.description || "-",
        [t("common.notes")]: supplier.notes || "-",
        [t("common.createdAt")]: supplier.createdAt
          ? new Date(supplier.createdAt).toLocaleDateString()
          : "-",
        [t("common.updatedAt")]: supplier.updatedAt
      ...
      toast({
        title: t("suppliers.exportSuccess"),
        description: t("suppliers.suppliersExportedCsv", { count: filteredSuppliers.length }),
      });
    } catch (error) {
      toast({
        title: t("common.error"),
        description: t("suppliers.exportFailed"),
        variant: "destructive",
      });
    }
  ...
  const exportToExcel = useCallback(async () => {
    try {
      ...
      worksheet.columns = [
        { header: t("suppliers.supplierName"), key: t("suppliers.supplierName"), width: 25 },
        { header: t("common.status"), key: t("common.status"), width: 12 },
        { header: t("common.description"), key: t("common.description"), width: 30 },
        { header: t("common.notes"), key: t("common.notes"), width: 30 },
      ...
      toast({
        title: t("suppliers.exportSuccess"),
        description: t("suppliers.suppliersExportedExcel", { count: filteredSuppliers.length }),
      });
    } catch (error) {
      toast({
        title: t("common.error"),
        ...
        <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
          ...
          >
            <SelectTrigger className="h-10 w-full sm:w-[180px] rounded-[28px] border border-emerald-400/30 dark:border-emerald-400/30 bg-gradient-to-r from-emerald-500/25 via-emerald-500/15 to-emerald-500/10 dark:from-emerald-500/25 dark:via-emerald-500/15 dark:to-emerald-500/10 text-gray-700 dark:text-white shadow-[0_10px_30px_rgba(16,185,129,0.2)] backdrop-blur-sm transition duration-200 hover:border-emerald-300/40 hover:from-emerald-500/35 hover:via-emerald-500/25 hover:to-emerald-500/15 dark:hover:border-emerald-300/40 dark:hover:from-emerald-500/35 dark:hover:via-emerald-500/25 dark:hover:to-emerald-500/15 font-medium">
              <SelectValue placeholder={t("suppliers.allSuppliers")} />
            </SelectTrigger>
