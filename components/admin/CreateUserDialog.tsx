...
const ROLE_VALUES = ["user", "admin", "supplier", "client", "retailer"] as const;
              ...
              >
                {ROLE_VALUES.map((role) => (
                  <SelectItem
                    key={role}
                    value={role}
                    className={`cursor-pointer focus:bg-blue-100 dark:focus:bg-white/10 focus:text-gray-900 dark:focus:text-white`}
                  >
                    {t(`admin.role.${role}`)}
                  </SelectItem>
                ))}
