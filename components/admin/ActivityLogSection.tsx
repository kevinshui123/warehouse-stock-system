...
export default function ActivityLogSection({
  ...
  const columns = useMemo<ColumnDef<AuditLog>[]>(
    () => [
      {
        id: "adminUser",
        header: t("admin.activityLog.adminUser"),
        cell: ({ row }) => {
      ...
      {
        id: "entity",
        header: t("admin.activityLog.entity"),
        cell: ({ row }) => {
          const log = row.original;
      ...
      {
        id: "activityDetails",
        header: t("admin.activityLog.activityDetails"),
        cell: ({ row }) => (
      ...
      {
        id: "when",
        header: t("admin.activityLog.when"),
        cell: ({ row }) => (
    ...
    >
      <div className="flex flex-col gap-4 mb-4">
        <div className="flex flex-col">
          ...
          <p className="text-sm sm:text-base text-gray-600 dark:text-gray-400">
            {t("admin.activityLog.description", {
              period:
                period === "today"
                  ? t("admin.activityLog.last24Hours")
                  : period === "7days"
                    ? t("admin.activityLog.last7Days")
                    : t("admin.activityLog.last30Days"),
            })}
          </p>
        </div>
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="relative flex-1 sm:max-w-md">
            ...
            <Input
              placeholder={t("admin.activityLog.searchByUser")}
              value={searchTerm}
      ...
      {isPending && logs.length === 0 ? (
        ...
        <p className="text-sm sm:text-base text-gray-500 dark:text-gray-500 py-6 text-center">
          {searchTerm.trim()
            ? t("admin.activityLog.noMatchingActivity")
            : t("admin.activityLog.noActivityInPeriod")}
        </p>
      ) : (
