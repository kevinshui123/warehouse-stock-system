...
const InvoiceList = React.memo(
  ({ detailHrefBase, dataSource = "invoices" }: InvoiceListProps = {}) => {
    ...
    const columns = useMemo(
      () =>
        createInvoiceColumns(t, handleEditInvoice, effectiveDetailBase, {
          showSourceBadge: dataSource === "adminCombined",
          showIssuedBy: isClientInvoicesPage,
        }),
      [t, handleEditInvoice, effectiveDetailBase, dataSource, isClientInvoicesPage],
    );
      ...
      <div className="flex flex-col poppins">
        ...
        <div className="pb-6 flex flex-col items-start text-left">
          <h2 className="text-xl sm:text-2xl font-semibold text-gray-900 dark:text-white pb-2">
            {isAdminCombined
              ? t("invoices.storeInvoicesTitle")
              : isClientInvoices
                ? t("invoices.clientInvoicesTitle")
                : isClientInvoicesPage
                  ? t("client.myInvoices")
          ...
          <p className="text-sm sm:text-base text-gray-600 dark:text-gray-400">
            {isAdminCombined
              ? t("invoices.storeInvoicesDesc")
              : isClientInvoices
                ? t("invoices.clientInvoicesDesc")
                : isClientInvoicesPage
                  ? t("invoices.yourInvoicesDesc")
                  : t("invoices.invoiceManagementDesc")}
          </p>
        </div>
