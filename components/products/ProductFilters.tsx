...
export default function FiltersAndActions({
    ...
    try {
      if (filteredProducts.length === 0) {
        toast({
          ...
          description: t("products.noDataToExport"),
          variant: "destructive",
        });
        return;
      }
      ...
      const csvData = filteredProducts.map((product) => ({
        ...
        [t("products.category")]: product.category || t("products.unknown"),
        [t("products.supplier")]: product.supplier || t("products.unknown"),
        [t("common.createdAt")]: new Date(product.createdAt).toLocaleDateString(),
      }));
      ...
      toast({
        title: t("products.exportCsvSuccess"),
        description: t("products.productsExportedCsv", { count: filteredProducts.length }),
      });
    } catch (error) {
      toast({
        title: t("common.error"),
        description: t("products.exportFailed"),
        variant: "destructive",
      });
    }
  ...
  const exportToExcel = useCallback(async () => {
    try {
      ...
      // Add header row
      worksheet.columns = [
        { header: t("products.productName"), key: t("products.productName"), width: 20 },
        { header: t("products.sku"), key: t("products.sku"), width: 15 },
        { header: t("products.price"), key: t("products.price"), width: 10 },
        { header: t("products.quantity"), key: t("products.quantity"), width: 10 },
        { header: t("common.status"), key: t("common.status"), width: 12 },
        { header: t("products.category"), key: t("products.category"), width: 15 },
        { header: t("products.supplier"), key: t("products.supplier"), width: 15 },
      ...
      toast({
        title: t("products.exportExcelSuccess"),
        description: t("products.productsExportedExcel", { count: filteredProducts.length }),
      });
    } catch (error) {
      toast({
        title: t("common.error"),
      ...
      {productOwnerOptions && onOwnerChange && (
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-center gap-2 sm:gap-3 w-full">
          <span className="text-sm font-medium text-gray-700 dark:text-white/80 flex items-center gap-2">
            ...
            {t("products.selectProductOwner")}
          </span>
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" className={exportButtonClass + " w-full sm:w-auto"}>
                {selectedOwnerId
                  ? productOwnerOptions.find((a) => a.id === selectedOwnerId)?.name ?? t("products.productOwner")
                  : t("products.productOwner")}
