...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate categoryId and supplierId are non-empty strings
    ...
    // Check for Prisma ObjectID validation errors
    if (
      error instanceof Error &&
      error.message.includes("Malformed ObjectID")
    ) {
      return NextResponse.json(
        { error: "Invalid category or supplier selection. Please ensure both are selected." },
        { status: 400 },
      );
    }
